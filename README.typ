...
=== Двусвязный список (выбрано)

*Преимущества:*
- Возможность обхода в обе стороны
- Эффективное удаление элементов O(1) при наличии указателя на узел
- Динамическое изменение размера без необходимости перераспределения памяти
- Отсутствие ограничения на количество элементов

*Недостатки:*
- Больший расход памяти на хранение ссылок (два указателя на узел)
- Медленный доступ к произвольному элементу (O(n))
...
- Необходимость перераспределения памяти при расширении
...
Цели разработки:
- Создать консольное приложение для управления текстовыми заметками
...
- *Стандартная библиотека C++ (STL)*: Используется для всех операций с данными и файлами
- Отсутствие зависимостей от внешних библиотек обеспечивает простоту развертывания и портативность
...
- Динамическое управление памятью без ограничения на количество заметок
...
Для хранения коллекции заметок используется двусвязный список:
...
*Обоснование выбора двусвязного списка:*
- Динамическое изменение размера без ограничений на количество заметок
- Эффективное добавление новых заметок в конец списка (O(1))
- Эффективное удаление заметок (O(1) при наличии указателя на узел)
- Возможность двунаправленного обхода списка
- Упрощение операций вставки и удаления по сравнению с массивом
...
*Назначение в программе:* Алгоритм отвечает за создание новой заметки в системе, включая валидацию входных данных, создание файла заметки и добавление узла в двусвязный список.

*Используемый метод:* Последовательное выполнение операций с применением линейного поиска для проверки уникальности названия и использованием двусвязного списка для хранения данных.

*Входные параметры:*
- `title` (string) - название заметки (1-100 символов)
- `category` (string) - тема заметки (1-50 символов)
- `content` (string) - текст заметки (до 10000 символов)

*Возвращаемые параметры:*
- `bool` - true при успешном добавлении, false при ошибке (например, дублирование названия)

*Описание шагов алгоритма:*
+ Получить от пользователя название, тему и текст заметки
+ Проверить уникальность названия путем обхода двусвязного списка
+ Сгенерировать уникальный ID
+ Создать структуру Note с текущей датой
+ Сохранить текстовый файл заметки
+ Создать новый узел NoteNode и добавить его в конец двусвязного списка
+ Обновить указатели tail и увеличить счетчик заметок
+ Сохранить метаданные в файл

#figure(
  // Место для блок-схемы алгоритма добавления заметки
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма добавления новой заметки_
    ]
  ],
  caption: [Блок-схема алгоритма добавления новой заметки]
) <fig:add-note>

=== Алгоритм поиска заметок по теме

*Назначение в программе:* Алгоритм предназначен для фильтрации и отображения заметок, принадлежащих к определенной теме/категории.

*Используемый метод:* Линейный поиск с последовательным обходом двусвязного списка заметок и сравнением категорий.

*Входные параметры:*
- `category` (string) - название темы для поиска (1-50 символов)

*Возвращаемые параметры:*
- Отображение найденных заметок в консоли (или сообщение об отсутствии результатов)
- Внутренне: массив/список найденных заметок

*Описание шагов алгоритма:*
+ Получить тему для поиска от пользователя
+ Выполнить линейный поиск по двусвязному списку заметок
+ Начать с головы списка (head) и пройти по указателям next
...
#figure(
  // Место для блок-схемы алгоритма поиска по теме
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма поиска заметок по теме_
    ]
  ],
  caption: [Блок-схема алгоритма поиска заметок по теме]
) <fig:search-category>

=== Алгоритм удаления заметки

*Назначение в программе:* Алгоритм обеспечивает безопасное удаление заметки из системы, включая удаление файла и обновление структуры двусвязного списка.

*Используемый метод:* Поиск узла по ID с последующим удалением из двусвязного списка и удалением соответствующего файла.

*Входные параметры:*
- `note_id` (int) - уникальный идентификатор заметки для удаления

*Возвращаемые параметры:*
- `bool` - true при успешном удалении, false при ошибке (например, заметка не найдена)

*Описание шагов алгоритма:*
+ Получить ID заметки для удаления
+ Найти узел заметки в двусвязном списке
+ Запросить подтверждение удаления
+ Удалить текстовый файл заметки
+ Обновить указатели prev и next соседних узлов
+ Освободить память узла
+ Обновить указатели head/tail при необходимости
+ Сохранить обновленные метаданные в файл

#figure(
  // Место для блок-схемы алгоритма удаления заметки
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма удаления заметки_
    ]
  ],
  caption: [Блок-схема алгоритма удаления заметки]
) <fig:delete-note>

=== Алгоритм загрузки заметок из файла

*Назначение в программе:* Алгоритм обеспечивает восстановление состояния системы при запуске путем загрузки метаданных и содержимого заметок из файловой системы.

*Используемый метод:* Последовательное чтение файла метаданных с построением двусвязного списка заметок.

*Входные параметры:*
- Нет (чтение из файла `notes_metadata.dat`)

*Возвращаемые параметры:*
- `bool` - true при успешной загрузке, false при ошибке
- Побочный эффект: заполнение двусвязного списка заметок

*Описание шагов алгоритма:*
+ Открыть файл метаданных `notes_metadata.dat`
+ Прочитать каждую строку файла
+ Разобрать строку на поля (ID, название, тема, дата, путь к файлу)
+ Создать структуру Note с полученными данными
+ Создать новый узел NoteNode и добавить в конец списка
+ Обновить счетчик заметок и указатель на следующий ID
+ Закрыть файл метаданных

#figure(
  // Место для блок-схемы алгоритма загрузки заметок
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма загрузки заметок из файла_
    ]
  ],
  caption: [Блок-схема алгоритма загрузки заметок из файла]
) <fig:load-notes>

=== Алгоритм отображения всех заметок

*Назначение в программе:* Алгоритм предназначен для вывода списка всех заметок в табличном формате на экран пользователя.

*Используемый метод:* Последовательный обход двусвязного списка от головы до хвоста с форматированным выводом.

*Входные параметры:*
- Нет (использует внутренний двусвязный список заметок)

*Возвращаемые параметры:*
- Нет (вывод в консоль)

*Описание шагов алгоритма:*
+ Проверить наличие заметок (если noteCount == 0, вывести сообщение)
+ Установить указатель current на голову списка (head)
+ Пока current не равен nullptr:
  - Вывести ID заметки с форматированием
  - Вывести название заметки (обрезать до 22 символов при необходимости)
  - Вывести категорию заметки (обрезать до 14 символов при необходимости)
  - Вывести дату создания
  - Перейти к следующему узлу (current = current->next)

#figure(
  // Место для блок-схемы алгоритма отображения заметок
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма отображения всех заметок_
    ]
  ],
  caption: [Блок-схема алгоритма отображения всех заметок]
) <fig:display-notes>

=== Алгоритм сохранения метаданных в файл

*Назначение в программе:* Алгоритм обеспечивает сохранение метаданных всех заметок в файл для обеспечения персистентности данных между сеансами работы программы.

*Используемый метод:* Последовательный обход двусвязного списка с записью данных в текстовый файл в формате CSV.
...
*Описание шагов алгоритма:*
+ Открыть файл `notes_metadata.dat` для записи
+ Установить указатель current на голову списка (head)
+ Пока current не равен nullptr:
  - Записать в файл строку формата: `id|title|category|date|filepath`
  - Перейти к следующему узлу (current = current->next)
+ Закрыть файл
+ Вернуть результат операции

#figure(
  // Место для блок-схемы алгоритма сохранения метаданных
  rect(width: 100%, height: 200pt, stroke: 1pt + gray, fill: rgb("#f0f0f0"))[
    #align(center + horizon)[
      _Место для блок-схемы алгоритма сохранения метаданных в файл_
    ]
  ],
  caption: [Блок-схема алгоритма сохранения метаданных в файл]
) <fig:save-metadata>
...
void NoteManager::displayAllNotes() const {
    ...
    // Обход двусвязного списка от головы до хвоста
...
- *Эффективные структуры данных:* Двусвязный список обеспечивает быстрое добавление (O(1)) и удаление (O(1)) элементов
- *Оптимизация памяти:* Использование ссылок для передачи больших объектов, динамическое выделение памяти только для существующих заметок
- *Прямой доступ:* Хранение указателей на голову и хвост списка для быстрого добавления в конец
...
- *Цель:* Проверить работу системы с большим количеством заметок
- *Метод:* Поэтапное добавление множества заметок для проверки производительности
- *Результат:* Благодаря использованию двусвязного списка, система эффективно обрабатывает добавление и удаление заметок без ограничения на их количество. Операции добавления и удаления выполняются за O(1) при наличии указателя на узел
- *Вывод:* Двусвязный список обеспечивает эффективное масштабирование системы
...
+ *Нефункциональные требования:*
  ...
  - Масштабируемость: отсутствие жестких ограничений на количество заметок благодаря использованию двусвязного списка

+ *Качество кода:*
  - Отсутствие утечек памяти благодаря корректному управлению динамической памятью
...
+ *Технические достижения:*
  ...
  - Использована структура данных двусвязный список для динамического управления коллекцией заметок
...
- *Эффективность:* Быстрое добавление и удаление заметок благодаря двусвязному списку (O(1))
- *Масштабируемость:* Отсутствие ограничений на количество заметок
...
- *Линейный поиск:* Поиск по ID и категории требует обхода списка (O(n))
...
+ *Улучшение работы с данными:*
  ...
  - Оптимизация поиска с использованием хэш-таблицы или индексирования
...
Применение двусвязного списка в качестве основной структуры данных обеспечивает гибкость и масштабируемость системы, позволяя эффективно управлять неограниченным количеством заметок. Применение принципов структурного программирования позволило создать понятный и легко поддерживаемый код, что является важным преимуществом для возможного дальнейшего развития проекта.
...
+ Мейерс, С. Эффективное использование C++. 55 верных советов улучшить структуру и код ваших программ / С. Мейерс. — М.: ДМК Пресс, 2006. — 240 с.
+ Кормен, Т. Алгоритмы: построение и анализ / Т. Кормен, Ч. Лейзерсон, Р. Ривест, К. Штайн. — М.: Вильямс, 2013. — 1328 с.
+ Документация C++ Reference [Электронный ресурс]. — Режим доступа: https://en.cppreference.com/
+ Стандарт языка C++17 [Электронный ресурс]. — Режим доступа: https://isocpp.org/
...
```cpp
// note.h - Заголовочный файл структуры заметки
#ifndef NOTE_H
#define NOTE_H
...
int main() {
    ...
    do {
        ...
        switch(choice) {
            ...
            case 2: manager.displayAllNotes(); break;
            case 3: manager.searchByCategory(); break;
            case 4: manager.openNote(); break;
