...
*Двусвязный список*
*Преимущества:*
- Эффективное добавление и удаление элементов (O(1) при наличии прямого указателя на узел)
- Возможность обхода в обе стороны
- Удобство удаления элементов

*Недостатки:*
- Медленный доступ к произвольному элементу по индексу (O(n))
...
Для данной задачи выбран двусвязный список, так как он обеспечивает эффективное добавление и удаление заметок, а также позволяет легко обходить элементы в обоих направлениях.
...
Для хранения коллекции заметок используется двусвязный список (NoteNode), что обеспечивает:

Эффективное добавление и удаление элементов без необходимости перемещения других элементов
Динамическое изменение размера при добавлении и удалении элементов
Возможность обхода списка в обоих направлениях
...
```cpp void NoteManager::searchByCategory(const std::string& category) const {
    bool foundAny = false;
    ...
    // Обход списка и поиск по категории
    ...
    while (current != nullptr) {
        if (current->data.category == category) {
            ...
            std::cout << current->data.category << " | ";
...
4. Эффективность: Для хранения данных в памяти выбран двусвязный список, обеспечивающий хорошую производительность для операций добавления и удаления.
...
[13], [Поиск по теме (найдено)], [Существующая тема], [Вывод найденных заметок], [PASSED],
[14], [Поиск по теме (не найдено)], [Несуществующая тема], [Сообщение "Не найдено"], [PASSED],
)
)
...
4. Выбор методов и алгоритмов: Проанализированы возможные методы решения, выбраны оптимальные структуры данных (двусвязный список) и алгоритмы (линейный поиск).
