...
8. Обновление коллекции: Добавить объект Note в список заметок в оперативной памяти.

=== Алгоритм поиска заметок по теме

1. Ввод темы: Получить от пользователя тему для поиска.
2. Линейный поиск: Пройти по всем элементам списка notes.
3. Сравнение: Для каждой заметки сравнить поле category с искомым значением.
4. Вывод результатов: При совпадении сразу вывести заметку в табличном формате.
5. Завершение: После обхода всего списка, если ничего не найдено, вывести сообщение об отсутствии совпадений.

=== Алгоритм удаления заметки

1. Ввод ID: Получить от пользователя идентификатор заметки для удаления.
2. Поиск заметки: Найти заметку с указанным ID в списке notes.
...
6. Удаление из памяти: Удалить узел из списка notes.
